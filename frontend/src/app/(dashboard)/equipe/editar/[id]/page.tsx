...
import { getMembroEquipe, updateMembroEquipe } from "@/lib/api/membros_equipe";
...
export default function EditarMembroPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [membro, orcamentosData] = await Promise.all([
          getMembroEquipe(id),
          getOrcamentos(),
        ]);
        ...
        if (membro) {
          ...
          setDataInicio(membro.data_inicio);
          setDescricao(membro.descricao || "");
          setOrcamentoId(membro.orcamento_id || "—");
          setStatus(membro.status);
          
          // Format salary for the input
          const formattedVal = membro.remuneracao.toLocaleString("pt-BR", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          });
          setRemuneracao(formattedVal);
        }
      } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Ocorreu um erro ao salvar as alterações do colaborador.");
    }
  };

  if (loading) {
    return (
      <div className="bg-white border border-[#f1f5f9] rounded-[8px] p-16 text-center flex flex-col items-center justify-center gap-4 min-h-[300px]">
    ...
    <div className="flex flex-col gap-6 max-w-[1024px] mx-auto pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="font-['JetBrains_Mono'] text-[10px] font-medium text-[#00a3b1] uppercase tracking-[0.5px] leading-[15px]">
            Gestão / Recursos Humanos / Editar Colaborador
          </p>
      ...
      >
        <div className="p-8 flex flex-col gap-8">
          ...
          {/* Section 2: Alocação, Atividades e Status */}
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
              <div className="bg-[#9fd300] h-[24px] w-[6px] rounded-[9999px]" />
              <h3 className="font-['JetBrains_Mono'] font-medium text-[12px] text-[#001b3d] tracking-[0.6px] uppercase">
