...
export function useUserRole() {
    ...
    useEffect(() => {
        ...
        const fetchRole = async (currentUser: User) => {
            ...
            if (lastUserRef.current === currentUser.id && lastRoleRef.current !== null) {
                return lastRoleRef.current;
            }

            try {
                // 1. Try metadata
                let userRole = currentUser.app_metadata?.role || currentUser.user_metadata?.role

                // 2. If no role in metadata, try 'profiles' table
                if (!userRole) {
                    try {
