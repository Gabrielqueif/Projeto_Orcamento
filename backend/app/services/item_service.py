...
# Ajustar path se necessário. backend/app/services -> backend
...
class ItemService:
    ...
    def _limpar_link_excel(self, texto):
    ...
    def _gerar_chave_match(self, texto: str) -> str:
        texto = self._limpar_link_excel(texto)
        sem_acento = self._remover_acentos(texto)
        return re.sub(r'[^a-z0-9]', '', sem_acento)

    def _limpar_valor_moeda(self, valor):
    ...
    def importar_sinapi(self) -> Dict[str, Any]:
        if not PLANILHA_SINAPI_CAMINHO.exists():
            raise FileNotFoundError("Arquivo planilha_sinapi.xlsx não encontrado.")
        ...
        try:
            ...
            # 1. LOCALIZAR ABAS
            aba_analitico = next((a for a in abas if "analitico" in self._normalizar_nome_aba(a) and "custo" not in self._normalizar_nome_aba(a)), None)
            if not aba_analitico: aba_analitico = next((a for a in abas if "analitico" in self._normalizar_nome_aba(a)), None)

            aba_precos = next((a for a in abas if "csd" in self._normalizar_nome_aba(a)), None)
            if not aba_precos: aba_precos = next((a for a in abas if "ccd" in self._normalizar_nome_aba(a)), None)
            if not aba_precos: aba_precos = next((a for a in abas if "cse" in self._normalizar_nome_aba(a)), None)
            ...
            if aba_analitico:
                ...
                for r_idx, row in df_raw.head(30).iterrows():
                    row_clean = [self._remover_acentos(str(v)) for v in row.values]
                ...
                if idx_row_header != -1 and col_idx_cod != -1:
                    start_idx = int(idx_row_header) + 1 if isinstance(idx_row_header, (int, float, str)) and str(idx_row_header).isdigit() else 0
                    for r_idx in range(start_idx, len(df_raw)):
                        row = df_raw.iloc[r_idx]
                        
                        if col_idx_tipo != -1:
                            tipo = self._remover_acentos(str(row.iloc[col_idx_tipo]))
                        ...
                        if cod.isdigit() and cod != '0':
                            chave = self._gerar_chave_match(desc)
                            if len(chave) > 5:
            ...
            if aba_precos:
                ...
                for r_idx, row in df_grid.head(15).iterrows():
                    vals = [self._remover_acentos(str(v)).strip() for v in row.values]
                ...
                start_search = int(row_idx_estados) + 1 if isinstance(row_idx_estados, int) and row_idx_estados != -1 else 0
                ...
                if row_idx_dados_inicio != -1 and mapa_col_estados_temp:
                    ...
                    for _, row in df_dados.iterrows():
                        ...
                        chave = self._gerar_chave_match(desc_raw)
                        ...
                        if cod_recuperado:
                            ...
                            for c_idx, sigla_estado in mapa_col_estados_temp.items():
                                val = row.iloc[c_idx]
                                val_float = self._limpar_valor_moeda(val)
            ...
            registros_comp = self._dedup(registros_comp)
            registros_precos = self._dedup(registros_precos)

            q_comp = self.repository.upsert_batch_composicoes(registros_comp)
            q_est = self.repository.upsert_batch_estados(registros_precos)
